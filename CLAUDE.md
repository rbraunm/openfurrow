...
- **Reproducible by default (decision 0004).** Every trial layout is regenerable from a
...
- **Own the core math; oracles validate, they do not compute (decision 0005).** The
...
- **Open, portable storage (decision 0003).** The canonical store is a single SQLite
...
- **Fail loud, no silent fallbacks.** Invalid data is rejected at the boundary it enters.
  Missing observation cells are rejected (decision 0007), not silently imputed. No
  second-strategy fallback runs behind a failed primary path.

## Match the field, configurably (decisions 0008, 0009)
